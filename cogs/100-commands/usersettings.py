...
from utils.userdata import get_available_settings, get_setting_type, change_user_setting

def _get_choices_from_list_settings():
    return [app_commands.Choice(name=x,value=x) for x in get_available_settings()]
...
class UserSettingsCog(commands.GroupCog, group_name="usersettings"):
    ...
    async def set(self, interaction: discord.Interaction, setting: app_commands.Choice[str], value: str):
        ...
        setting_type = get_setting_type(setting_val)
        if setting_type is bool:
            ...
            return
        change_user_setting(interaction.user.id, setting_val, value_typed)
