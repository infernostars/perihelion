...
def color_hsv(value: float, min_value: float = 0.0, max_value: float = 1.0) -> discord.Color:
    """
    Interpolates a color between red (0.0) and green (1.0) in the HSV color space.
    There are also special exceptions for the minimum and maximum value.
    ...
    #...unless there's a max!
    if value == max_value:
        saturation = 1
        brightness = 1
        hue = 200
    elif value == min_value: # or a min
        saturation = 0
        brightness = 0
        hue = 0
